...
const { clinic, treatments, features, symptoms, news } = require('./data');
...
<section class="section">
  <div class="container">
    ...
    <div class="clinic-gallery">
      <figure class="cg-item cg-large"><div class="cg-ph">受付・待合室</div><figcaption>清潔感のある明るい受付・待合室</figcaption></figure>
      <figure class="cg-item"><div class="cg-ph">診療室</div><figcaption>プライバシーに配慮した診療室</figcaption></figure>
      <figure class="cg-item"><div class="cg-ph">カウンセリング</div><figcaption>じっくり相談できるカウンセリングスペース</figcaption></figure>
      <figure class="cg-item"><div class="cg-ph">キッズスペース</div><figcaption>お子様も安心のキッズスペース</figcaption></figure>
      <figure class="cg-item"><div class="cg-ph">パウダールーム</div><figcaption>身だしなみを整えるパウダーコーナー</figcaption></figure>
    </div>
  </div>
</section>
<section class="section section-alt">
  <div class="container">
    ...
    <div class="equip-grid">
      <div class="equip-card"><h3>徹底した滅菌・消毒</h3><p>治療器具は用途に応じて滅菌・消毒を徹底。手に触れる部分の消毒も丁寧に行い、清潔な環境を保ちます。</p></div>
      <div class="equip-card"><h3>精密な診断機器</h3><p>歯科用CTやマイクロスコープなどの先進機器を導入し、正確な診断と見落としのない丁寧な診療に努めます。</p></div>
      <div class="equip-card"><h3>快適な院内環境</h3><p>定期的な換気と清掃で、いつでも気持ちよくお過ごしいただける空間を整えています。</p></div>
    </div>
    ...
    <div class="device-grid">
      ...
      { name: 'ジェットウォッシャー（ミーレ）', en: 'Miele Washer', desc: '国際規格（ISO 15883）に準拠した全自動洗浄消毒器。冷水で血液などの蛋白汚れを洗浄後、93℃の熱水（Ao値3000）で器具の細部まで熱水消毒・高レベル除菌します。' },
...
<section class="section">
  <div class="container">
    <div class="doctor-intro">
      <div class="doctor-photo"><img src="assets/images/staff/staff-001.png" alt="院長"></div>
      <div class="doctor-body">
        <p class="doctor-role">院長</p>
        <p class="doctor-name">${clinic.director}<span class="doctor-kana">${clinic.directorKana}</span></p>
        <div class="doctor-message">
          <p>ホームページをご覧いただき、誠にありがとうございます。院長の${clinic.director}と申します。</p>
          <p>当院では、患者様に「来てよかった」「気持ちよく過ごせた」と感じていただける歯科医院を目指しています。歯科治療に不安や恐怖心をお持ちの方も少なくありません。だからこそ、痛みに最大限配慮し、一人ひとりのペースに寄り添った、無理のない診療を大切にしています。</p>
...
{
  const depth = 0;
  const priceTable = (caption, rows) => `<div class="price-block">
    <h3 class="price-cat">${caption}</h3>
    <table class="price-table">
      ${rows.map(r => `<tr><th>${r[0]}</th><td class="price-val">${r[1]}</td><td class="price-note">${r[2] || ''}</td></tr>`).join('\n')}
    </table>
  </div>`;
  const body = `
${renderPageHero('Price', '料金表', '自由診療の料金の目安をご案内します。詳しくは診察時にご説明いたします。', { depth, crumbs: [{ label: '料金表' }] })}
<section class="section">
  <div class="container narrow">
    ...
    ['セラミックインレー（詰め物）', '¥44,000〜', ''],
    ['オールセラミッククラウン（被せ物）', '¥88,000〜', ''],
  ])}
    ${priceTable('矯正歯科', [
    ['マウスピース矯正（部分）', '¥330,000〜', ''],
    ['マウスピース矯正（全体）', '¥770,000〜', ''],
    ...
    ${priceTable('インプラント', [
    ['インプラント（1歯）', '¥385,000〜', '手術・上部構造込'],
  ])}
    <p class="price-disclaim">※上記は目安の金額です。お口の状態により変動します。詳しい費用は、検査・診断のうえで丁寧にご説明いたします。<br>※お支払いには各種クレジットカードもご利用いただけます。</p>
  </div>
</section>`;
  write('price.html', renderPage({
    title: `料金表｜${clinic.name}`,
    description: `${clinic.name}の自由診療の料金の目安。ホワイトニング・セラミック・矯正・インプラントなどの費用をご案内します。`,
